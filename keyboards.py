...
markup_start = ReplyKeyboardMarkup(resize_keyboard=True).add(
    button_hi_1).add(button_hi_2).add(button_hi_3)

markup_advice = ReplyKeyboardMarkup(resize_keyboard=True).add(button_hi_4).add(
    button_hi_5).add(button_hi_6).add(button_hi_7).add(button_hi_9)

inline_btn_1 = InlineKeyboardButton(
    text='Что делает эта кнопка?',
    url='https://www.youtube.com/watch?v=dQw4w9WgXcQ&list=PLxb0XwjhqM_RLETkiOkUZrEE8K-fP3V-p&index=3')
